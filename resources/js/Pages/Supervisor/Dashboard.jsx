...
import { Calendar as CalendarIcon, Clock, MapPin, CheckCircle, AlertCircle, PlayCircle, FileText, MailCheck } from 'lucide-react';

export default function Dashboard({ stats = {}, kegiatanKalender = [], tabData = {}, undanganKonfirmasi = [], auth }) {
      ...
      <div className="flex-1 min-w-0 bg-[#F5F7FA] min-h-screen md:ml-64">
        ...
        <main className="pt-20 md:pt-28 px-4 pb-12">
          <div className="w-full">
            ...
            {/* Card List Undangan Memerlukan Konfirmasi (Full Width Solid Card, Tampil Jika Ada) */}
            {undanganKonfirmasi && undanganKonfirmasi.length > 0 && (
              <div className="w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100">
                  <div className="flex items-center space-x-2">
                    ...
                    <h2 className="text-base font-bold text-gray-900">Undangan Memerlukan Konfirmasi</h2>
                    <span className="text-xs bg-orange-50 text-orange-700 border border-orange-200 font-semibold px-2.5 py-0.5 rounded-full">
                      {undanganKonfirmasi.length} Undangan Perlu Ditinjau
                    </span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {undanganKonfirmasi.map((item) => (
                    <div
                      key={item.id}
                      className="bg-white border border-gray-200 hover:border-orange-300 rounded-xl p-4 hover:shadow-md transition-all flex flex-col justify-between"
                    >
                      <div>
                        ...
                        <span className="text-xs text-gray-500 block truncate mt-0.5 font-normal">
                          {item.judul_kegiatan} • <span className="text-gray-600 font-medium">Pembuat: {item.pembuat}</span>
                        </span>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">
                        ...
                        <Link
                          ...
                          className="inline-flex items-center px-3.5 py-1.5 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-semibold text-xs rounded-lg shadow-sm transition-all transform hover:scale-105"
                        >

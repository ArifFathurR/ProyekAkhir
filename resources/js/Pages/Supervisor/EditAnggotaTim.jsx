...
export default function EditAnggotaTim({ anggota_tim, anggotaTim, users = [], tims = [] }) {
  const targetAnggota = anggota_tim || anggotaTim || {};

  const getNormalizedRole = (role) => {
    if (!role) return '';
    const lowerRole = role.toLowerCase();
    if (lowerRole.includes('ketua') || lowerRole.includes('leader')) return 'Ketua Tim';
    if (lowerRole.includes('anggota') || lowerRole.includes('member')) return 'Anggota';
    return role;
  };

  const { data, setData, put, processing, errors } = useForm({
    user_id: targetAnggota.user_id || '',
    tim_id: targetAnggota.tim_id || '',
    role: getNormalizedRole(targetAnggota.role),
  });
  ...
  const handleSubmit = (e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        put(route('supervisor.updateAnggotaTim', targetAnggota.id), {
          onSuccess: () => {
            Swal.fire({
              title: 'Berhasil!',
          ...
          onError: (errs) => {
            ...
            Swal.fire({
              title: 'Gagal!',
              text: 'Gagal memperbarui data. Mohon periksa kembali isian form.',
              icon: 'error',
              showConfirmButton: false,
              timer: 2500,
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => router.get(route('supervisor.anggota_tim'))}

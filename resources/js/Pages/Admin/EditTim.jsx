...
export default function EditTim({ tim }) {
  // Initialize the form data based on the `tim` prop
  const { data, setData, put, processing, errors } = useForm({
    nama_tim: tim?.nama_tim || '',  // Safely access tim.nama_tim
  });

  // Handle form submission
  const handleSubmit = (e) => {
    e.preventDefault();
    put(route('tim.update', tim.id));  // Ensure this route matches your route in web.php
  };
      ...
      <div className="flex-1 bg-[#F5F7FA] min-h-screen md:ml-64">
        ...
        <main className="pt-28 px-6">
          <div className="bg-white shadow rounded p-8 mx-auto">
            <h2 className="text-2xl font-semibold text-center mb-6">Formulir Update Tim</h2>

...
export default function ModalEditUndangan({
  ...
  // Load data when undangan prop changes or modal shows
  useEffect(() => {
    if (show && undangan) {
      clearErrors();
      setData({
        kegiatan_id: undangan.kegiatan_id ? String(undangan.kegiatan_id) : '',
        ...
        tim_ids: [],
        user_ids: undangan.penerima_undangan ? undangan.penerima_undangan.map(p => p.user_id) : [],
      });

      if (undangan.penerima_undangan) {
        const initialSelected = undangan.penerima_undangan
          .filter(p => p.user)
          .map(p => ({
  ...
  const pegawaiSelectOptions = useMemo(() => 
    pegawaiList.map(p => ({
      value: String(p.id),
  ...
  const handleTimSelectChange = (selectedOptions) => {
    setSelectedTims(selectedOptions || []);
    ...
    if (hasAll) {
      const allPegawai = pegawaiSelectOptions;
      const allTimIds = tims.map(t => String(t.id));
      setSelectedPegawai(allPegawai);
      setData('user_ids', allPegawai.map(p => p.value));
      setData('tim_ids', allTimIds);
      return;
    }

    const selectedTimIds = selectedOptions.map(opt => Number(opt.value));
    const matchedUserIds = anggotaTim
      .filter(item => selectedTimIds.includes(Number(item.tim_id)))
      .map(item => String(item.user_id));

    const matchedPegawaiOptions = pegawaiSelectOptions.filter(p => matchedUserIds.includes(p.value));
    setSelectedPegawai(matchedPegawaiOptions);
    setData('user_ids', matchedPegawaiOptions.map(p => p.value));
    setData('tim_ids', selectedTimIds);
  };

  const handlePegawaiSelectChange = (selectedOptions) => {
    const selected = selectedOptions || [];
    setSelectedPegawai(selected);
    setData('user_ids', selected.map(p => p.value));
  };
    ...
    <Modal show={show} onClose={onClose} maxWidth="xl">
      <div className="p-5 sm:p-6 max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between border-b pb-3 mb-4">
          <h2 className="text-lg font-bold text-gray-900">Edit Undangan Kegiatan</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1.5">
              ...
              {selectedKegiatanObj && (
                <p className="text-xs text-blue-600 font-medium mt-1">
                  📅 Tanggal Kegiatan: <span className="font-bold">{selectedKegiatanObj.tanggal}</span>
                  {selectedKegiatanObj.tanggal_selesai ? ` s/d ${selectedKegiatanObj.tanggal_selesai}` : ''}
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Survei / Sensus">Survei / Sensus</SelectItem>
          ...
          {/* Pilihan Penerima: Berdasarkan Tim */}
          <div className="space-y-1.5">
            <Label className="text-sm">Pilih Tim Penerima (Otomatis memilih seluruh anggota tim)</Label>
            <ReactSelect
              isMulti
              options={timOptions}
              value={selectedTims}
              onChange={handleTimSelectChange}
              placeholder="Pilih Tim untuk undang semua anggotanya..."
              className="react-select-container text-sm"
              classNamePrefix="react-select"
            />
          </div>

          {/* Pilihan Penerima: Pegawai Spesifik */}
          <div className="space-y-1.5">
            <Label className="text-sm">Penerima Spesifik (Pegawai) <span className="text-red-500">*</span></Label>
            <ReactSelect
              isMulti
              options={pegawaiSelectOptions}
              value={selectedPegawai}
              onChange={handlePegawaiSelectChange}
              placeholder="Cari & pilih nama pegawai..."
              className="react-select-container text-sm"
              classNamePrefix="react-select"
            />
            {errors.user_ids && <div className="text-red-500 text-xs mt-0.5">{errors.user_ids}</div>}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <Button
              type="submit"
              disabled={processing}
              className="bg-[#0B2E74] hover:bg-blue-800 text-white px-6 font-semibold"
            >
